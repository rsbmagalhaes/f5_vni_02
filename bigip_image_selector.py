...
def get_public_images(region):
    ...
    try:
        ...
        sys.stderr.write('Can not fetch F5 image catalog at %s: %s' %
                         (catalog_url, ex))
        sys.exit(1)
...
def longest_substr(type, catalog_image_name, version_prefix):
    ...
    if catalog_image_name.find(version_prefix) < 0:
        return 0
    seqMatch = difflib.SequenceMatcher(None, catalog_image_name,
                                       version_prefix)
    match = seqMatch.find_longest_match(0, len(catalog_image_name), 0,
                                        len(version_prefix))
    return match.size
...
def get_image(region, type, version):
    ...
    for image in image_catalog[region]:
        if not image['image_id']:
            continue
        match_length = longest_substr(type, image['image_name'], version)
...
def main():
    ...
    if tmos_type not in ALLOWED_TMOS_TYPES:
        sys.stderr.write('TMOS type must be in: %s' % ALLOWED_TMOS_TYPES)
        sys.exit(1)
    tmos_version_match = jsondata['version_prefix'].lower().replace('.', '-')
    region = jsondata['region'].lower()
    if region not in PUBLIC_REGIONS:
        sys.stderr.write(
            'public f5 images are not supported in region: %s. Use a custom VPC image.'
            % region)
        sys.exit(1)
    (image_url, image_name, image_id) = get_image(region, tmos_type,
                                                  tmos_version_match)
    if not image_url:
        sys.stderr.write(
            'No image in the public image catalog matched version %s' %
            tmos_version_match)
        sys.exit(1)
